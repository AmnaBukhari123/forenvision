...
export default function CaseDetail() {
  ...
  const handleRunDetection = async (runOnAll = false) => {
    ...
    try {
      ...
      if (response.ok) {
        setDetectionMessage({ type: "success", text: result.message || "Object detection completed successfully" });
        await loadDetectionResults();
        setTimeout(() => {
        ...
        setDetectionMessage({ type: "error", text: result.detail || "Detection failed. Please try again." });
      }
    } catch (error) {
      setDetectionMessage({ type: "error", text: error.message || "Object detection failed. Please try again." });
    } finally {
      setRunningDetection(false);
    }
  };
  ...
  const handleDeleteDetectionResult = async (resultId) => {
    if (!window.confirm("Are you sure you want to delete this detection result?")) return;
  ...
  const handleDeleteEvidence = async (evidenceId, filename) => {
    if (!window.confirm(`Are you sure you want to delete "${filename}"? This will also delete any associated detection results.`)) return;
  ...
  const getModelName = (modelType) => {
    if (modelType === 'blood' || modelType === 'blood_detection') return 'Blood Detection';
    ...
    <div className="case-detail-container">
          ...
          {!isEditing ? (
            ...
            <div className="edit-mode-header">
              ...
              <div className="edit-actions">
                <button className="btn-save-edit" onClick={handleSaveEdit} disabled={updating}>
          ...
          {witnessStatements.length === 0 ? (
            ...
            witnessStatements.map((witness) => (
              <div key={witness.id} className="witness-card">
                <button className="delete-witness-btn" onClick={() => handleDeleteWitnessStatement(witness.id)} disabled={deletingWitnessId === witness.id} title="Delete witness statement">
                  {deletingWitnessId === witness.id ? '⏳' : '×'}
                </button>
                <div className="witness-header">
                  <h4 className="witness-name">{witness.witness_name}</h4>
                  {witness.statement_date && (
                    <span className="witness-date">📅 {new Date(witness.statement_date).toLocaleDateString()}</span>
                  )}
                </div>
                ...
                <div className="witness-statement"><p>{witness.statement}</p></div>
                <div className="witness-footer">
                  <small className="witness-timestamp">Added: {new Date(witness.created_at).toLocaleString()}</small>
                </div>
              </div>
            ))
          )}
      ...
      <section className="analysis-tools-section">
        ...
        <div className="analysis-tools-grid">
          <div className="analysis-tool-card">
            ...
            <button onClick={handleOpenDetectionModal} className="tool-btn primary" disabled={imageEvidence.length === 0}>
              {imageEvidence.length === 0 ? 'No Images' : 'Run Detection'}
            </button>
            <small className="tool-hint">{imageEvidence.length} image{imageEvidence.length !== 1 ? 's' : ''} available</small>
          </div>
          ...
          <div className="analysis-tool-card">
            ...
            <button className="tool-btn tertiary" onClick={handleGenerateReport} disabled={generatingReport}>
              {generatingReport ? (<><span className="loading-spinner"></span> Generating...</>) : "Generate Report"}
            </button>
        ...
        <div className="section-header">
          ...
          <div className="section-header-actions">
            ...
            <div className="upload-controls">
              <input ref={fileInputRef} type="file" onChange={(e) => setFile(e.target.files[0])} className="file-input" id="evidence-file-input" style={{ display: 'none' }} />
            ...
            <div className="evidence-grid">
              {evidence.map((ev) => (
                <div key={ev.id} className="evidence-card">
                  <button className="delete-evidence-btn" onClick={() => handleDeleteEvidence(ev.id, ev.filename)} disabled={deletingEvidenceId === ev.id} title="Delete evidence">
                    {deletingEvidenceId === ev.id ? '⏳' : '×'}
                  </button>
                  <div className="evidence-preview">
                    {ev.filename.toLowerCase().match(/\.(jpg|jpeg|png|gif|bmp|webp)$/) ? (
                      <img src={`http://127.0.0.1:8000/uploads/${ev.filename}`} alt={ev.filename} className="evidence-image" onError={(e) => { e.target.style.display = 'none'; e.target.nextSibling.style.display = 'flex'; }} />
                    ) : null}
                    <div className="file-icon" style={{ display: ev.filename.toLowerCase().match(/\.(jpg|jpeg|png|gif|bmp|webp)$/) ? 'none' : 'flex' }}>
                      {ev.filename.toLowerCase().match(/\.(pdf)$/) ? '📄' : ev.filename.toLowerCase().match(/\.(mp4|avi|mov)$/) ? '🎥' : ev.filename.toLowerCase().match(/\.(doc|docx)$/) ? '📝' : '📁'}
                    </div>
                  </div>
                  <div className="evidence-info">
                    <a href={`http://127.0.0.1:8000/uploads/${ev.filename}`} target="_blank" rel="noreferrer" className="evidence-link">{ev.filename}</a>
            ...
            {detectionResults.map(result => {
              ...
              return (
                <div key={result.id} className="detection-result-horizontal-card">
                  <button className="delete-result-btn-horizontal" onClick={() => handleDeleteDetectionResult(result.id)} disabled={deletingResultId === result.id} title="Delete detection result">
                    {deletingResultId === result.id ? '⏳' : '×'}
                  </button>
                  <div className="detection-image-container">
                    {imagePath && (
                      <div className="detection-image-wrapper">
                        <img src={imagePath} alt={getEvidenceFilename(result.evidence_id)} className="detection-result-image" />
                        <svg className="detection-overlay" viewBox={`0 0 ${detectionData?.image_dimensions?.width || 640} ${detectionData?.image_dimensions?.height || 640}`}>
                          {detections.map((detection, idx) => {
                            ...
                            return (
                              <g key={idx}>
                                <rect x={bbox.x1} y={bbox.y1} width={bbox.x2 - bbox.x1} height={bbox.y2 - bbox.y1} fill="none" stroke={color} strokeWidth="3" opacity="0.8" />
                                <rect x={bbox.x1} y={bbox.y1 - 25} width={(detection.class_name.length * 8) + 20} height="25" fill={color} opacity="0.9" />
                                <text x={bbox.x1 + 5} y={bbox.y1 - 8} fill="white" fontSize="14" fontWeight="bold">
                    ...
                    <div className="detection-header-row">
                      <div>
                        ...
                        <div className="detection-meta">
                          {result.model_type && <span className="model-badge-horizontal">{getModelName(result.model_type)}</span>}
                          {detectionData?.total_detections > 0 && <span className="detection-count-badge">{detectionData.total_detections} objects detected</span>}
                        </div>
                      </div>
                      {detectionData?.image_dimensions && (
                        <div className="detection-stats-mini">
                          <div className="stat-mini">
                            <span className="stat-value-mini resolution-display">{detectionData.image_dimensions.width}×{detectionData.image_dimensions.height}</span>
                      ...
                      {detections.length > 0 ? (
                        detections.map((detection, index) => (
                          <div key={index} className="detection-chip">
                            ...
                            {detection.category && <span className="chip-category">{detection.category}</span>}
                          </div>
                        ))
                      ) : (
                        <div className="no-detections-horizontal"><span>⚠️ No objects detected in this image</span></div>
                      )}
                    </div>
                    {detectionData?.category_counts && Object.keys(detectionData.category_counts).length > 0 && (
                      <div className="category-summary">
                        ...
                        {Object.entries(detectionData.category_counts).map(([category, count]) => (
                          <span key={category} className="category-tag">{category} ({count})</span>
                        ))}
                      </div>
                    )}
                    <div className="detection-timestamp">📅 Analyzed: {new Date(result.created_at).toLocaleString()}</div>
                  </div>
                </div>
              );
            })}
      ...
      <section className="saved-reports-section">
        <div className="section-header">
          ...
          <span className="evidence-count">{savedReports.length} report{savedReports.length !== 1 ? 's' : ''}</span>
        </div>
              ...
              <div key={report.id} className="saved-report-card">
                <div className="saved-report-info">
                  <div className="saved-report-meta">
                    ...
                    {report.generated_by && <span className="report-meta-item">👤 {report.generated_by}</span>}
                  </div>
                  <div className="saved-report-timestamp">🕒 Generated: {new Date(report.created_at).toLocaleString()}</div>
                </div>
                <div className="saved-report-actions">
                  <button className="btn-view-report" onClick={() => handleViewSavedReport(report)}>👁 View</button>
                  <button className="delete-witness-btn" onClick={() => handleDeleteSavedReport(report.id)} disabled={deletingReportId === report.id} title="Delete report" style={{ position: 'static', width: '32px', height: '32px' }}>
        ...
        <div className="modal-overlay" onClick={() => setShowDetectionModal(false)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              ...
              <button className="modal-close" onClick={() => setShowDetectionModal(false)}>×</button>
            </div>
            <div className="modal-body">
              {detectionMessage.text && (
                <div className={`detection-message ${detectionMessage.type}`}>{detectionMessage.text}</div>
              )}
              <div className="detection-settings">
                <div className="form-group">
                  <label>Detection Model:</label>
                  <select value={selectedModel} onChange={(e) => setSelectedModel(e.target.value)} disabled={runningDetection} className="model-select">
                  ...
                  <small className="form-hint">
                    {selectedModel === 'blood' ? 'Specialized model for detecting blood and bloodstain evidence' : 'General model for weapons, evidence, and human elements'}
                  </small>
                </div>
                <div className="form-group">
                  <label>Select Evidence to Analyze:</label>
                  <select value={selectedEvidence} onChange={(e) => setSelectedEvidence(e.target.value)} disabled={runningDetection} className="evidence-select">
              ...
              <div className="detection-controls">
                <button className="btn-run-detection" onClick={() => handleRunDetection(false)} disabled={runningDetection || !selectedEvidence}>
      ...
      {showReportModal && (
        <div className="modal-overlay" onClick={() => !generatingReport && setShowReportModal(false)}>
          <div className={`modal-content report-modal-content ${reportFullscreen ? "report-modal-fullscreen" : ""}`} onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <h3>📄 Forensic Investigation Report</h3>
              <div className="report-modal-header-actions">
                <button className="modal-fullscreen-btn" onClick={() => setReportFullscreen(!reportFullscreen)} title={reportFullscreen ? "Minimize" : "Full Screen"}>
                  {reportFullscreen ? "⊡ Minimize" : "⛶ Full Screen"}
                </button>
                <button className="modal-close" onClick={() => { setShowReportModal(false); setReportFullscreen(false); }} disabled={generatingReport}>×</button>
              </div>
            </div>
            <div className="modal-body">
              {generatingReport && (
              ...
              {reportData && !generatingReport && (
                <>
                  ...
                  <div className="report-text-container">
                    <pre className="report-text">{reportData.report}</pre>
                  </div>
                </>
              )}
            </div>
